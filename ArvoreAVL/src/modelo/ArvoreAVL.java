...
public class ArvoreAVL {
	...
	private No inserir(No arv, int chave, String valor) {
		...
		/* atualiza altura do ancestral (pai) do nó inserido */
		arv.alturaNo = 1 + maior(this.altura(arv.getEsq()), 
				this.altura(arv.getDir()));
		
		/* Obter fator de balanceamento deste nó 
		 * para verificar se ele se tornou desbalanceado
		 * */
		
		int fb = this.obterFB(arv);
		int fbSubArvEsq = this.obterFB(arv.getEsq());
		int fbSubArvDir = this.obterFB(arv.getDir());
	...
	private No res(No x) {
		...
		x.alturaNo = 1 + this.maior(this.altura(x.getEsq()), 
				this.altura(x.getDir()));
		
		y.alturaNo = 1 + this.maior(this.altura(y.getEsq()), 
				this.altura(y.getDir()));
